...
def verify(root):
    ...
    for relative, digest in expected.items():
        path = root / relative
        if relative in {
            "src/driving_data/cli.py", "src/driving_data/evaluation.py",
            "tests/test_evaluation_integrity.py", "scripts/verify_artifacts.py", "README.md",
        }:
            archived = root / "docs/maintenance/2026-09-19/baseline" / f"{relative}.txt"
            # Synthetic fixtures still exercise the original verifier contract.
...
if __name__ == "__main__":
    ...
    print(f"Verified {count} historical source/config/evidence files; frozen manifest preserved; current code is tested separately")
