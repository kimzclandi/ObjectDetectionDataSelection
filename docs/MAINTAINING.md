...
Stack: **Python 3.12 / PyTorch / DuckDB**. Main paths: `src/driving_data/, scripts/, tests/, configs/, reports/`.
...
actual test and replay commands. The existing workflow and ignore rules are included in the frozen artifact manifest and
remain unchanged. The new documentation workflow has its own concurrency and timeout
settings. No inference or training coverage is implied.
